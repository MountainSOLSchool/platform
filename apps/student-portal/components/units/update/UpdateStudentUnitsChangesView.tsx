...
export function UpdateStudentUnitsChangesView(
    ...
    const addedRepeatableCompletions = props.repeatableCompletionChanges.filter(
        (c) => c.type === 'added'
    );
    const updatedRepeatableCompletions =
        props.repeatableCompletionChanges.filter((c) => c.type === 'updated');
    const removedRepeatableCompletions =
        props.repeatableCompletionChanges.filter((c) => c.type === 'removed');
                        ...
                        <div className="mb-3">
                            ...
                            <ul className="list-disc pl-5">
                                {addedRepeatableCompletions.map((change) => (
                                    <li
                                        key={`${change.completion.unitId}-${change.completion.recordedDate}`}
                                    >
                                        {change.unitName} (recorded on{' '}
                                        {formatDate(
